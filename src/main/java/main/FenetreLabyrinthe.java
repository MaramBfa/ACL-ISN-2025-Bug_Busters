...
public class FenetreLabyrinthe extends JPanel {
    ...
    private void verifierCollisions() {
        ...
        if (peutPrendreDegat) {
            ...
            if (hit) {
                if ("monstre".equals(typeEnnemi)) Sound.play("/sounds/monster_snarl.wav");
                else if ("zombie".equals(typeEnnemi)) Sound.play("/sounds/zombie_growl.wav");
                else if ("fantôme".equals(typeEnnemi)) Sound.play("/sounds/ghost_pass.wav");
        ...
        // --- Clé fixe dans la grille ---
        if (grille[hero.getX()][hero.getY()] == 'K') {
            grille[hero.getX()][hero.getY()] = ' '; // case redevient vide
            hero.pickKey();
            hero.ajouterScore(10);
        ...
        // --- Porte fixe dans la grille ---
        if (grille[hero.getX()][hero.getY()] == 'D') {
            if (!hero.hasKey()) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < grille.length; i++) {
            for (int j = 0; j < grille[0].length; j++) {
                ...
                // Clé fixe
                if (grille[i][j] == 'K') {
                    g2.drawImage(keyImg, j * TAILLE_CASE, i * TAILLE_CASE, null);
                }

                // Porte fixe
                if (grille[i][j] == 'D') {
                    g2.drawImage(doorImg, j * TAILLE_CASE, i * TAILLE_CASE, null);
                }
            }
        }
